...
class DeformableTransformer(nn.Module):
    ...
    def forward(self, srcs, masks, pos_embeds, query_embed):
        ...
        hs, hs_lang, inter_references = self.decoder(
            query_embed,
            reference_points,
            memory,
            spatial_shapes,
        ...
        return (hs, hs_lang, init_reference_out, inter_references_out, enc_outputs_class, enc_outputs_coord_unact)
...
class DeformableCompositeTransformerDecoder(nn.Module):
    def __init__(
        ...
        self.projs = nn.ModuleList([
            MLP(self.vis_dim, self.lang_dim, self.lang_dim, 2)
            for _ in range(num_layers)
        ])
        for mlp in self.projs:
    ...
    def forward(
        ...
        for lid, layer in enumerate(self.layers):
            ...
            output_text_lang = self.projs[lid](output)
            ...
            if self.return_intermediate:
                ...
                intermediate_lang.append(output_text_lang)
                intermediate_reference_points.append(reference_points)

        if self.return_intermediate:
            return torch.stack(intermediate), torch.stack(intermediate_lang), torch.stack(intermediate_reference_points)

        return output, output_text_lang, reference_points
