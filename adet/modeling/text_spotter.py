...
class TransformerPureDetector(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        weight_dict = {
            ...
            'loss_plm':loss_cfg.POINT_LM_WEIGHT 
            # 'loss_infonce':loss_cfg.POINT_TEXT_WEIGHT
        }
        ...
        if cfg.MODEL.TRANSFORMER.BOUNDARY_HEAD:
            # dec_losses = ["labels", "ctrl_points",'plm', "bd_points"]
            dec_losses = ["labels", "texts", "ctrl_points",'plm', "bd_points"]
        else:
            # dec_losses = ["labels", "ctrl_points",'plm','infonce']
            dec_losses = ["labels", "texts", "ctrl_points",'plm','infonce']
    ...
    def forward(self, batched_inputs):
        ...
        if self.training:
            ...
            ctrl_point_text = output["pred_langs"]
    ...
    def run_inference_w_BART(self, model, vis_embeds, max_len=27):
        prev_training = model.training
        model.eval()
        final_logits = []
        num_inst = vis_embeds.size(0)
        pad_token_id = model.config.pad_token_id
        bos_token_id = model.config.decoder_start_token_id
        eos_token_id = model.config.eos_token_id

        for i in range(num_inst):
            encoder_hidden = vis_embeds[i].unsqueeze(0)  # [1, L, D]
            generated = torch.full((1, 1), bos_token_id, dtype=torch.long, device=vis_embeds.device)
            ...
            for _ in range(max_len):
                logits = model(
                    ...
                    attention_mask=None,  # or real attn_mask if needed
                )  # [1, T, vocab]
            ...
            final_logits.append(instance_logits)

        # pad to max length
        max_T = max(logits.size(1) for logits in final_logits)
        padded = []
        for logits in final_logits:
            pad_len = max_T - logits.size(1)
            if pad_len > 0:
                pad_tensor = torch.zeros((1, pad_len, logits.size(2)), device=logits.device)
                logits = torch.cat([logits, pad_tensor], dim=1)
            padded.append(logits)

        final_logits_tensor = torch.cat(padded, dim=0)  # [N, max_T, vocab]
        final_logits_softmax = torch.softmax(final_logits_tensor, dim=-1)
        _, topi = final_logits_softmax.topk(1, dim=-1)
        model.train(prev_training)
        return topi.squeeze(-1)  # [N, max_T]
    ...
    def inference(
        ...
        with torch.no_grad():
            ...
            bs,n_ob ,max_len,lang_dim = ctrl_point_text.shape
            
            prob = ctrl_point_cls.mean(-2).sigmoid()
            scores, labels = prob.max(-1)
            if bd_points is not None:
                for scores_per_image, labels_per_image, ctrl_point_per_image, text_per_image, bd, image_size in zip(
                    ...
                    bd = bd[selector]
                    '''
                    Updated with fully utilizing BART 
                    '''
                    ...
                    bd[..., 1::2] *= image_size[0]
                    result.bd = bd
                    if text_per_image.size(0) == 0:
                        print("WARNING: No text predictions for this image, assigning empty result.") 
                        beam_output_wo_spec = torch.full((0, max_len), self.criterion.lang_model.config.pad_token_id, dtype=torch.long, device=text_per_image.device)
                        result.recs = beam_output_wo_spec
                    else:       
                        result.recs = self.run_inference_w_BART(self.criterion.lang_model, text_per_image,100) # [N, T, Lv]     
                ...
                for scores_per_image, labels_per_image, ctrl_point_per_image, text_per_image, image_size in zip(
                    ...
                    if text_per_image.size(0) == 0:
                        ...
                        result.recs = self.run_inference_w_BART(self.criterion.lang_model, text_per_image,max_len=27) # [N, T, Lv]     
