...
from .language import get_language_model_BART_dec,get_language_tokenizer_BART
...
class SetCriterion(nn.Module):
    ...
    def __init__(
        ...
        self.lang_model = get_language_model_BART_dec()
        self.lang_tokenizer = get_language_tokenizer_BART()

    def train_plm(self,lang_embs,target_texts):

        bos = torch.full((target_texts.size(0), 1), self.lang_tokenizer.bos_token_id, device=lang_embs.device)
        decoder_input_ids = torch.cat([bos, target_texts[:, :-1]], dim=1)
        ...
        if torch.isnan(logits).any() or torch.isinf(logits).any():
            print('plm logit is nan')
            return torch.tensor(0.0, device=lang_embs.device)
        else:
            plm_loss = F.cross_entropy(logits.transpose(1, 2), target_texts.long())
            if torch.isnan(plm_loss).any() :
                print('plm loss is nan')
                torch.tensor(0.0, device=lang_embs.device)
            else:
                return plm_loss

    def loss_plm(self, outputs, targets, indices, num_inst):
        assert 'pred_langs' in outputs
        idx = self._get_src_permutation_idx(indices)
        src_lang_embs = outputs['pred_langs'][idx]

        if src_lang_embs.numel() == 0 :
            print('src_lang_embs is empty')
            return {'loss_plm': torch.tensor(0.0, device=src_lang_embs.device)}

        target_texts_idx = torch.cat([t['texts'][i] for t, (_, i) in zip(targets, indices)], dim=0) 
        plm_loss = self.train_plm(src_lang_embs,target_texts_idx)
        return {'loss_plm':plm_loss}
    ...
    def loss_texts(self, outputs, targets, indices, num_inst):
        ...
        idx = self._get_src_permutation_idx(indices)
        # used for direct token-level classification 
        src_vis_embs = outputs['pred_text_logits'][idx]
        target_texts_idx = torch.cat([t['texts'][i] for t, (_, i) in zip(targets, indices)], dim=0)   
        if src_vis_embs.numel() == 0 :
            print('src_vis_embs is empty')
            return {'loss_texts': torch.tensor(0.0, device=src_vis_embs.device)}
        if torch.isnan(target_texts_idx).any() or torch.isinf(target_texts_idx).any():
            print('target_texts_idx is empty')
            return {'loss_texts': torch.tensor(0.0, device=src_vis_embs.device)}
        # text loss for token-level prediction
        token_pred_loss = F.cross_entropy(src_vis_embs.transpose(1, 2), target_texts_idx.long())
        return  {'loss_texts': token_pred_loss}
    ...
    def forward(self, outputs, targets):
        ...
        indices = self.dec_matcher(outputs_without_aux, targets, self.lang_model)
        ...
        losses = {}
        self.dec_losses = list(set(self.dec_losses) - set(['texts']))    
        ...
        if 'aux_outputs' in outputs:
            for i, aux_outputs in enumerate(outputs['aux_outputs']):
                indices = self.dec_matcher(aux_outputs, targets, self.lang_model)
                for loss in self.dec_losses:
                    kwargs = {}
                    if loss == 'labels':
