...
class DETECTION_TRANSFORMER(nn.Module):
    def __init__(self, cfg, backbone):
        ...
        # self.ctrl_point_text = nn.Linear(self.d_model, self.voc_size)  # specific character class for each point
        ...
        # self.ctrl_point_text.bias.data = torch.ones(self.voc_size) * bias_value
    ...
    def forward(self, samples: NestedTensor):
        ...
        for lvl in range(hs.shape[0]):
            ...
            outputs_langs.append(hs_lang[lvl])
            # outputs_text = self.ctrl_point_text[lvl](hs[lvl])  # bs, n_proposal, n_pts, voc_size
            outputs_text = torch.tensor(0, dtype=torch.int8)
        ...
        outputs_lang = torch.stack(outputs_langs) 
    ...
    @torch.jit.unused
    def _set_aux_loss(self, outputs_class, outputs_text, outputs_coord, outputs_lang, outputs_bd_coord):
        if outputs_bd_coord is not None:
            return [
                {'pred_logits': a, 'pred_text_logits': b, 'pred_ctrl_points': c, 'pred_langs': d ,'pred_bd_points': e}
                for a, b, c, d, e in zip(outputs_class[:-1], outputs_text[:-1], outputs_coord[:-1], outputs_lang[:-1], outputs_bd_coord[:-1])
            ]
        else:
            return [
                {'pred_logits': a, 'pred_text_logits': b, 'pred_ctrl_points': c, 'pred_langs': d }
                for a, b, c, d in zip(outputs_class[:-1], outputs_text[:-1], outputs_coord[:-1], outputs_lang[:-1])
            ]